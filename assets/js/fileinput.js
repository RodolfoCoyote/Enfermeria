...
(function (factory) {
  ...
  FileInput.prototype = {
    ...
    _setZoomContent: function ($frame, navigate) {
      ...
      /* AQUI HICE LAS MODIFICACIONES */
      tmplt = $zoomPreview.attr("data-template") || "generic";
      $content = $zoomPreview.find(".kv-file-content");
        let divImgContent = $($content[0]).find('img'); // Encuentra .kv-file-content dentro de $content[0]
        let imgDataZoom = $(divImgContent[0]).data('zoom');
        let newImgZoom = `<img src="${imgDataZoom}" class="kv-preview-data file-preview-image">`;

        // Establece 'body' a la nueva imagen
      body = $content.length ? newImgZoom : "";
